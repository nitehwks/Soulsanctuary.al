...
export function PrivacyDashboard({ isOpen, onClose, userId }: PrivacyDashboardProps) {
  ...
  const { data: summary, isLoading } = useQuery<PrivacySummary>({
    ...
    queryFn: async () => {
      const res = await apiFetch(`/api/privacy/summary/${userId}`);
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed right-0 top-0 h-full w-[400px] max-w-[100vw] bg-background border-l border-border shadow-2xl z-50 flex flex-col"
